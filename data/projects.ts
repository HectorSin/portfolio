...
const baseProjects: Omit<Project, "id" | "slug" | "detail">[] = [
  ...
  },
  {
    ...
    featuredSubtitle: {
      en: "MP3 splitting system built by replacing an LLM-first approach with waveform analysis",
      ko: "LLM 중심 접근 대신 파형 분석으로 해결한 듣기평가 MP3 자동 분할 시스템",
    },
    featuredResultLabel: {
      en: "Result",
      ko: "대표 성과",
    ...
    keyContributions: {
      en: [
        "Reframed the problem from LLM timestamping to signal-based boundary detection",
        "Implemented waveform analysis, silence detection, and FFmpeg splitting pipeline",
        "Built validation checks for question order, count, and timestamp intervals",
      ],
      ko: [
        "LLM 타임스탬프 추출에서 신호 기반 경계 탐지로 접근 전환",
        "파형 분석, 무음 탐지, FFmpeg 자동 분할 파이프라인 구현",
        "문항 순서, 개수, 타임스탬프 간격 검증 로직 구축",
      ],
    },
    previewImageSrc: "/projects/automated-mp3-splitting-listening-assessments/uml-system-architecture.svg",
    previewImageAlt: {
      en: "Audio splitting system architecture diagram",
      ko: "음원 자동 분할 시스템 아키텍처 다이어그램",
    },
    ...
    description: {
      en: "Identified the limit of LLM-based timestamping and replaced it with waveform analysis, building an automated MP3 splitting system that processed 206 files with zero production errors.",
      ko: "LLM 기반 타임스탬프 추출의 한계를 확인하고 파형 분석 방식으로 전환해, 듣기평가 MP3 206개를 무오류로 처리한 자동 분할 시스템을 구축했습니다.",
    },
    achievements: {
      en: [
        "Automatically split 206 listening-assessment files with zero errors",
        "Processed 30-minute source audio in about 10 to 15 seconds",
        "Replaced one-hour manual editing work per file with an automated FFmpeg pipeline",
        "Detected question boundaries using silence windows and waveform pattern changes",
        "Reduced outsourcing costs by 100%, saving more than KRW 2 million annually",
      ],
      ko: [
        "듣기평가 MP3 206개 파일을 무오류로 자동 분할",
        "30분 음원도 약 10~15초 내 처리",
        "파일당 1시간 이상 걸리던 수작업 편집을 FFmpeg 자동화로 대체",
        "무음 구간과 파형 변화 기반 문항 경계 탐지 알고리즘 구현",
        "외주 비용 100% 절감 및 연간 200만 원 이상 비용 절감",
      ],
    },
